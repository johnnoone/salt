...
def parse(query, parse_matcher):
    python_stmt = to_python(query)
    return eval(python_stmt, {'parse_matcher': parse_matcher})
...
class QueryState(ParsingState):
    ...
    @transition
    def matcher(self, data, builder):
        builder.append('parse_matcher(' + repr(data) + ')')
        return pushing(GroupState)
...
class GroupState(ParsingState):
    ...
    def matcher(self, data, builder):
        raise ValueError(
            'Statement is missing before {0} matcher'.format(repr(data))
        )
